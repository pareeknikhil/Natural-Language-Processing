...
def get_features(preprocessed_snippet) :
	# comment the code below for part 4 of the homework
	feature_vector = np.zeros(len(vocabulary_dict))
	# uncomment the code below for part 4 of the homework
	# feature_vector = np.zeros(len(vocabulary_dict)+3)
	# dal = load_dal("dict_of_affect.txt")
	# snippet_score = score_snippet(preprocessed_snippet,dal)
	...
	# uncomment the code below for part 4 of the homework
	# feature_vector[-3] = snippet_score[0];
	# feature_vector[-2] = snippet_score[1];
	# feature_vector[-1] = snippet_score[2];
...
def get_key_from_vocabulary(index) :
	if(index == len(vocabulary_dict)) :
		return "lexicon_activeness"
	elif(index == len(vocabulary_dict)+1) :
		return "lexicon_pleasantness"
	elif(index == len(vocabulary_dict)+2) :
		return "lexicon_imagery"
	else :
...
def load_dal(dal_path) :
	...
	while line :
		parts_of_line = line.split("\t")
		score_dictionary[parts_of_line[0]] = tuple(float(i) for i in parts_of_line[1:])
		line = file.readline().strip("\n")
	return score_dictionary

def score_snippet(preprocessed_snippet, dal) :
	sum_of_scores = (0.0,0.0,0.0)
	count_of_words = 0.0
	for index,(word,pos) in enumerate(preprocessed_snippet) :
		current_score = 1
		#process the word if it does not contain the edit tag
		if(word.find("EDIT_") == -1) :
			if(word.find("NOT_") != -1) :
				current_score = -1
				word = word.strip("NOT_")
			if(dal.get(word) != None) :
				count_of_words = count_of_words + 1
				current_tuple = tuple(map(lambda x: x*current_score, dal.get(word)))
				sum_of_scores = tuple(map(operator.add, sum_of_scores, current_tuple))
	if(count_of_words !=0) :
		return tuple(map(lambda x: x/count_of_words, sum_of_scores))
	else :
...
if __name__ == '__main__':
	...
	#uncomment the code below for part 4 of the homework
	# X_train = np.empty([len(snippet_label_list),len(vocabulary_dict)+3])
	...
	#uncomment the code below for part 4 of the homework
	# X_test = np.empty([len(snippet_label_test_list),len(vocabulary_dict)+3])
